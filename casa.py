...
class CasaScraper():
    ...
    def scrape_listings(self):
        page_num = 2  # Start from the first page

        while True:
            # Update the URL with the current page number
            paginated_url = self.search_url.replace("lista-1", f"lista-{page_num}")
            ...
            for idx, listing in enumerate(listings, start=1):
                ...
                listing_file = self.listings_dir / f"idealista-{listing_id}.json"
...
if __name__ == "__main__":
    immobiliare_scraper = CasaScraper(search_url=search_url, listings_dir="listings")
